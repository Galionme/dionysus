...
import (
	...
	libpng "github.com/Galionme/dionysus/lib/png"
	"github.com/Galionme/dionysus/model"
	"github.com/Galionme/dionysus/util"
...
var (
	...
	opts  model.CmdLineOpts
	png   libpng.MetaChunk
)
...
func usage() {
	fmt.Fprintf(os.Stderr, "Example Usage: %s -i sys.png -o out.png --inject --offset 0x85258 --payload 1234\n", os.Args[0])
	fmt.Fprintf(os.Stderr, "Example Encode Usage: %s -i sys.png -o encode.png --inject --offset 0x85258 --payload 1234 --encode --key secret\n", os.Args[0])
...
func main() {
	...
	bReader, err := util.PreProcessImages(dat)
	if err != nil {
		log.Fatal(err)
	}
